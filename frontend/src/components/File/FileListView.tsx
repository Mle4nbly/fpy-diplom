...
export const FileListView = ({ name, size, uploadedAt, path }: FileListViewProps) => {
  const type = getFileType(path)
  
  return (
    <tr>
      <td className="d-flex align-items-center p-0">
        <div className="file-image me-2">
          {type === "image" ? (
      ...
      <td className="p-0">{(size / 1024 / 1024).toFixed(2)} MB</td>
      <td className="p-0">{new Date(uploadedAt).toLocaleString('ru-RU', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
        hour: '2-digit',
